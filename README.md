...
**Pipeline timing (cold, default direct-dispatch path):** forward 2026 ~35-55 s · backtest 2020 ~35-55 s · **warm/cached repeats <50 ms** (orchestrator in-memory cache). The pipeline is **accuracy-first**: extraction always waits for and consumes weather + geo + Tavily evidence on every call, so its driver weighting and recommendations always incorporate public-web signal. Wallclock is whatever Tavily takes (cache hit ~100 ms; cold network 6-10 s) plus extraction + feature. Set `NEXT_PUBLIC_DEMO_FAST=false` to fall back to the legacy GPT tool-use routing path (~80 s).
